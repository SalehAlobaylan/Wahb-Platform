...
describe('RUX collector', () => {
  ...
  it('assigns a stable page_load_id and monotonic sequence', () => {
    initCollector();
    emitEvent({ event_type: 'session_started', surface: 'pods' });
    emitEvent({ event_type: 'feed_requested', surface: 'pods' });
  ...
  it('flushes a critical event immediately (no timer wait)', () => {
    ...
    initCollector();
    emitEvent({ event_type: 'playback_failed', surface: 'pods', measurements: { failure_class: 'media_error' } });
  ...
  it('does not flush a single non-critical success event synchronously', () => {
    ...
    initCollector();
    emitEvent({ event_type: 'playback_started', surface: 'pods' });
  ...
  it('goes quiet for the session when the server reports disabled', async () => {
    ...
    initCollector();
    emitEvent({ event_type: 'feed_failed', surface: 'pods' }); // critical → flush
